...
function controller(fng, processArgs, options) {
    ...
    // });
    //
    fng.app.get.apply(fng.app, processArgs(fng.options, [':model/:id/version/:version', function (req, res) {
            getVersion(fng.getResource(req.params.model).model, req.params.id, req.params.version, function (err, obj) {
                if (err) {
...
function getAuditTrail(modelName, id, callback) {
    exports.Audit.find({ c: modelName, cId: id }).sort({ _id: -1 }).exec(function (err, trail) {
        if (err) {
            return callback(err);
        }
...
function getVersion(model, id, version, callback) {
    model.findOne({ _id: id }, function (err, latest) {
        ...
        exports.Audit.find({ c: model.modelName, cId: id, ver: { $gte: parseInt(version, 10) } }, { ver: 1, chg: 1 }, { sort: "-ver" }, function (err, histories) {
            ...
            var object = latest ? latest.toObject() : { _id: Mongoose.Types.ObjectId(id) };
...
function auditFromObject(doc, orig, updated, options, next) {
    ...
    var user = getPseudoField('user');
    var op = getPseudoField('op');
    ...
    if (chg) {
        ...
        exports.Audit.findOne({ c: c_1, cId: cId_1 }).sort("-ver").exec(function (err, prevAudit) {
            ...
            exports.Audit.create(auditRec, next);
        });
    }
    else {
        next();
        ...
        if (err) {
            ...
            async.eachSeries(results, function (currentObject, callback) {
                ...
                if (queryOp === 'findOneAndRemove') {
                    ...
                    original_1 = {};
                    if (queryObject.options && queryObject.options._user) {
                        original_1.__user = queryObject.options._user;
                    }
                    if (queryObject.options && queryObject.options._op) {
                        original_1.__op = queryObject.options._op;
                    }
                    Object.keys(queryObject._update).forEach(function (key) {
                        if (key[0] === '$') {
                            ...
                            // a simple assignment
                            original_1[key] = currentObject[key];
                            updated_1[key] = queryObject._update[key];
                        }
                    });
                }
